#!/bin/bash
# Builds OpenCodeStatusBar.app (and optionally a .dmg with: ./build.sh --dmg).
set -euo pipefail
cd "$(dirname "$0")"

# Ensure the plugin is compiled to CommonJS before bundling it.
...
# Bundle the plugin (so first-launch self-install works), the app icon, and the completion sound.
mkdir -p "$APP/Contents/Resources"
cp .opencode/plugins/opencode-status-bar.js "$APP/Contents/Resources/opencode-status-bar.js"
...
#        xcrun notarytool store-credentials "opencode-statusbar" \
...
NOTARY_PROFILE="${NOTARY_PROFILE:-opencode-statusbar}"
...
if [[ "${1:-}" == "--dmg" ]]; then
  ...
  echo "Packaging DMG…"
  DMG="build/OpenCodeStatusBar.dmg"
  ...
  # Eject any stale "OpenCode Status Bar" volumes from earlier builds first. Otherwise a name
  # collision mounts this one as "OpenCode Status Bar 2", the hardcoded /Volumes path below points
  # at the wrong volume (layout capture silently fails), and the stale mounts pile up in Finder.
  for d in $(hdiutil info | awk '/OpenCode Status Bar/ {print $1}'); do hdiutil detach "$d" >/dev/null 2>&1 || true; done

  # Lay out the window on a read-write image to capture its .DS_Store, then build the final
  # image from the folder (see below).
  hdiutil create -volname "OpenCode Status Bar" -srcfolder "$STAGE" -ov -format UDRW build/rw.dmg >/dev/null
...
tell application "Finder"
  tell disk "OpenCode Status Bar"
    ...
    set position of item "OpenCodeStatusBar.app" of container window to {130, 150}
...
OSA
  ...
  cp "/Volumes/OpenCode Status Bar/.DS_Store" "$STAGE/.DS_Store" 2>/dev/null || true
  ...
  hdiutil create -volname "OpenCode Status Bar" -srcfolder "$STAGE" -ov -format UDZO "$DMG" >/dev/null
  rm -rf "$STAGE"
  ...
  stray="$(find "/Volumes/OpenCode Status Bar" -maxdepth 1 -name ".*" ! -name ".DS_Store" 2>/dev/null)"
