...
All notable changes to OpenCode Status Bar are documented here. This project follows
...
- **OpenCode support.** The app and plugin have been adapted from Claude Code to OpenCode. The plugin now hooks into OpenCode's event model (`session.created`, `session.status`, `tool.execute.before/after`, `permission.asked/replied`) instead of Claude Code's hook system.
- New plugin file: `.opencode/plugins/opencode-status-bar.ts`, installed automatically to `~/.config/opencode/plugins/` on first launch.
- Unit tests for the plugin (`npm test`).

### Changed

- Status directory moved to `~/.local/state/opencode/statusbar/state.d/` (XDG state) and plugin config to `~/.config/opencode/plugins/` (XDG config).
- Simplified animation: a single glyph spinner replaces the Claude-specific spark and crab styles.
- App bundle renamed to `OpenCodeStatusBar.app` with bundle ID `com.local.opencodestatusbar`.
- Terminal-only support for this release; desktop app focus is planned for a future version.

### Removed

- Claude Code hook installer, `hooks/` scripts, and Claude-branded assets.
